...
describe("parsePolluantLabel", () => {
  it("keeps name context and extracts final chemical formula", () => {
    const parsed = parsePolluantLabel("Acetic acid (vinegar) (C2H4O2)");
    expect(parsed.name).toBe("Acetic acid (vinegar)");
    expect(parsed.formula).toBe("C2H4O2");
    expect(parsed.label).toBe("Acetic acid (vinegar) (C2H4O2)");
  });
});
